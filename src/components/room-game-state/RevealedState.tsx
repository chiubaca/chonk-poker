...
export function RevealedState() {
  ...
  const hasConsensus = winningChoices.length === 1 && maxVotes === players.length;
  ...
  return (
    <div className="card bg-base-200 border-4 border-base-300 rounded-3xl overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-primary/20 to-secondary/20 px-4 sm:px-6 py-6 text-center">
        ...
        {hasConsensus && (
          <p className="text-sm opacity-60 mt-2">Everyone agrees! That's rare and beautiful!</p>
        )}
      </div>

      <div className="card-body p-4 sm:p-6">
        <div className="space-y-4">
          {Object.entries(chonkImages).map(([chonkValue, { src, label, size }], index) => {
            const count = choicesByChonk[chonkValue] || 0;
            const playersWithChoice = players.filter((p) => p.choice === chonkValue);
              ...
              >
                <div className="p-4 sm:p-5">
                  <div className="flex items-center gap-4">
                    {/* Chonk image */}
                    <div className={`relative ${isWinner ? "animate-wiggle" : ""}`}>
                      <img
                        src={src}
                        alt={label}
        ...
        <div className="mt-6 p-4 bg-base-100 rounded-2xl border-2 border-base-300 text-center">
          <div className="text-5xl sm:text-6xl mb-3">{hasConsensus ? "🙌🏼" : "🤔"}</div>
