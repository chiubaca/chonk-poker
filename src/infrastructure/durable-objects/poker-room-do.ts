...
} from "@/features/poker/state-machine/planning-poker-machine";
...
export class PokerRoomObject extends DurableObject<Env> {
  ...
  async gameAction(event: PokerGameEvents) {
    const query = this.sql.exec(`SELECT game_state FROM poker_room WHERE id = 1`);
    ...
    const gameState = JSON.parse(record.game_state as string) as PlanningPokerMachineSnapshot;
    ...
    this.sql.exec(`UPDATE poker_room SET game_state = ? WHERE id = 1`, serialisedGameState);
