...
import type { PokerGameContext, PokerGameEvents } from "./planning-poker-machine.types";

export type PlanningPokerMachineType = typeof planningPokerMachine;

export type PlanningPokerMachineSnapshot = SnapshotFrom<PlanningPokerMachineType>;
...
export const planningPokerMachine = setup({
  ...
  states: {
    choosing: {
      on: {
        ...
        "player.choose": {
          actions: assign(({ context, event }) =>
            produce(context, (draft) => {
              const player = draft.players.find((p) => p.id === event.player.id);
              if (player) {
        ...
        "player.boot": {
          ...
          actions: assign(({ context, event }) =>
            produce(context, (draft) => {
              const playerToBoot = draft.players.find((p) => p.id === event.playerId);
              if (playerToBoot?.state === "locked-in") {
                draft.lockedInPlayers -= 1;
              }
              draft.players = draft.players.filter((p) => p.id !== event.playerId);
              draft.players = draft.players.map((player) => ({
                ...player,
                choice: undefined,
                state: "choosing" as const,
    ...
    lockedIn: {
      ...
      description: "All players have locked in their choices. More players cannot join.",
    },
    revealed: {
      // type: "final",
