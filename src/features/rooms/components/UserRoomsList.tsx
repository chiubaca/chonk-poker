      ...
      <div className="card bg-base-200 border-4 border-base-300 rounded-3xl overflow-hidden">
        <div className="card-body p-4 sm:p-6">
          <div className="text-center py-6">
            ...
            <p className="text-xs opacity-40 mt-1">Create or join a room to get started</p>
          </div>
        </div>
      </div>
    );
            ...
            .map((room: UserRoom) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-mono font-bold tracking-wider text-sm">{room.roomId}</div>
                    <div className="flex items-center gap-1.5">
                      <span
                        className={`w-2 h-2 rounded-full ${
                          room.status === "live" ? "bg-success animate-pulse" : "bg-base-content/30"
                        }`}
                      />
                      <span className="text-xs opacity-60 capitalize">{room.status}</span>
                    </div>
                  </div>
                </div>
